...
// TODO [Andy] more general than SpringXD requires, trim it down
/**
...
//					lexNumericLiteral(ch=='0');
				...
				case 0:
					...
					break;
				case '\\':
					throw new InternalParseException(new SpelParseException(expressionString,pos,XDDSLMessages.UNEXPECTED_ESCAPE_CHAR));
				default:
	...
	private void lexQuotedStringLiteral() {
		...
		while (!terminated) {
			...
			if (ch==0) {
				throw new InternalParseException(new SpelParseException(expressionString,start,XDDSLMessages.NON_TERMINATING_QUOTED_STRING));
			}
		}
		pos++;
	...
	private void lexDoubleQuotedStringLiteral() {
		...
		while (!terminated) {
			...
			if (ch==0) {
				throw new InternalParseException(new SpelParseException(expressionString,start,XDDSLMessages.NON_TERMINATING_DOUBLE_QUOTED_STRING));
			}
		}
		pos++;
		tokens.add(new Token(TokenKind.LITERAL_STRING, subarray(start,pos), start, pos));
...
//	: (DECIMAL_DIGIT)+ (INTEGER_TYPE_SUFFIX)?;
	...
	private void lexNumericLiteral(boolean firstCharIsZero) {
		...
		if (isChar('L','l')) {
			if (isReal) { // 3.4L - not allowed
				throw new InternalParseException(new SpelParseException(expressionString,start,XDDSLMessages.REAL_CANNOT_BE_LONG));
			}
	...
	private void pushHexIntToken(char[] data,boolean isLong, int start, int end) {
		if (data.length==0) {
			if (isLong) {
				throw new InternalParseException(new SpelParseException(expressionString,start,XDDSLMessages.NOT_A_LONG,expressionString.substring(start,end+1)));
			} else {
				throw new InternalParseException(new SpelParseException(expressionString,start,XDDSLMessages.NOT_AN_INTEGER,expressionString.substring(start,end)));
			}
		}
		if (isLong) {
